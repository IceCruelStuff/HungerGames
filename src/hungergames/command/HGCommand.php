    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool {
        ...
        switch (strtolower($args[0])) {
            ...
            case "add":
                ...
                $game = $args[1];
                if ($this->HGApi->gameResourceExists($game) || $this->HGApi->gameArenaExists($game)) {
            ...
            case "join":
                ...
                if ($this->HGApi->getStorage()->isPlayerSet($sender) || $this->HGApi->getStorage()->isPlayerWaiting($sender)) {
                    return false;
                }
                if ($this->HGApi->getGlobalManager()->getGameManager($game)->getStatus() === "running" || $this->HGApi->getGlobalManager()->getGameManager($game)->getStatus() === "reset") {
                    return false;
                }
                $this->HGApi->getGlobalManager()->getGameManager($game)->addWaitingPlayer($sender, true);
                $sender->sendMessage(Msg::color("&aJoining game!"));
                if ($this->HGApi->getGlobalManager()->getGameManager($game)->isWaiting) {
                    return false; // checks if task started
                }
